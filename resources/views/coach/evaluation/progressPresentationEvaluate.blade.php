...
@section('content')
    <div class="container mx-auto p-4">
        <div class="m-5">
            <h1 class="text-3xl font-bold">Student Progress Presentation Evaluation</h1>
        </div>

        <div class="p-4">
            <div class="flex flex-row mb-4">
                <label for="stuId" class="block font-semibold text-base text-gray-700 mr-2">Student Name:</label>
                <form action="{{ route('coaches.evaluations.progressPresentationEvaluate') }}" method="GET">
                            ...
                            @foreach ($plosByCourse[$courseCode] as $index => $plo)
                                <div id="ploEvaluation{{ $courseCode }}{{ $plo }}"
                                    ...
                                    <div id="studentEvaluations{{ $courseCode }}{{ $plo }}">
                                        ...
                                        <form
                                            action="{{ route('coaches.evaluations.progressPresentationEvaluate.store') }}"
                                            method="POST">
                                            @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var activeCourseTab = localStorage.getItem('activeCourseTab_progressPresentationEvaluateInd');
            var activePloTab = localStorage.getItem('activePloTab_progressPresentationEvaluateInd');
            if (activeCourseTab) {
        ...
        function switchCourseTab(courseCode) {
            ...
            localStorage.setItem('activeCourseTab_progressPresentationEvaluateInd', courseCode);
        ...
        function switchPloTab(courseCode, plo) {
            ...
            localStorage.setItem('activePloTab_progressPresentationEvaluateInd', plo);
        }
    </script>
@endsection
