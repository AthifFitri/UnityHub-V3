...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <div class="max-w-md mx-auto bg-gray-300 p-6 rounded-md shadow-md">
            <form action="{{ route('supervisors.profile.update') }}" method="POST">
                @csrf

                {{-- Staff Name --}}
                <div class="mb-4">
                    <label for="staffName" class="block font-medium text-base text-gray-700">Full Name</label>
                    <input type="text" name="staffName" id="staffName" class="block w-full rounded-md p-1"
                        value="{{ $supervisor->staffName }}" required>
                </div>

                {{-- Staff Phone Number --}}
                <div class="mb-4">
                    <label for="staffPhone" class="block font-medium text-base text-gray-700">Phone Number</label>
                    <input type="text" name="staffPhone" id="staffPhone" class="block w-full rounded-md p-1"
                        value="{{ $supervisor->staffPhone }}" required>
                </div>
                ...
                <div class="flex items-center justify-center mt-4">
                    ...
                    <a href="{{ route('supervisors.profile.index') }}"
