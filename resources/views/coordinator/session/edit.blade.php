...
@section('content')
    <div class="container mx-auto p-4">
        <div class="p-5 text-center">
            <h1 class="text-4xl font-bold">Edit Internship Session</h1>
        </div>

        <div class="max-w-md mx-auto bg-slate-300 p-6 rounded-md shadow-md">
            <form action="{{ route('coordinators.sessions.update', $session->sessionId) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="sessionSemester" id="sessionSemester"
                        ...
                        @foreach (range(1, 5) as $semester)
                            <option value="Sem {{ $semester }}" @if ($session->sessionSemester == "Sem $semester") selected @endif>Sem
                                {{ $semester }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="sessionYear" id="sessionYear"
                        class="block w-full rounded-md p-2 border border-gray-300 focus:outline-none focus:border-blue-500"
                        maxlength="9" required value="{{ $session->sessionYear }}">
                </div>
